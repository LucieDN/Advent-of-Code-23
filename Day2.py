...
evaluation_text="""Game 1: 8 green, 4 red, 4 blue; 1 green, 6 red, 4 blue; 7 red, 4 green, 1 blue; 2 blue, 8 red, 8 green
...
reference = {"red":12, "blue":14, "green":13}
#d1 = {"red":2, "blue":4}

#print(reference["red"])

def get_id(line): # Apply on "Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green" for example
    title = line.split(":")
    id = int("".join(list(filter(str.isdigit, title[0]))))
    return id

#print(get_id("Game 264: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue"))
def compare_dict(d1):
    for color in d1:
        if d1[color] > reference[color]:
            return False
    return True

#print(compare_dict(d1))

def get_results(revealed_cubes): # Apply on "3 blue, 4 red" for example
    sets = revealed_cubes.split(",")
    number_of_cubes = list(map(lambda text:int("".join(list(filter(str.isdigit, text)))), sets))
    cube_colors = list(map(lambda text:"".join(list(filter(str.isalpha, text))), sets))
    results = dict(zip(cube_colors, number_of_cubes))
    return results
    

#get_the_color(" 1 blue")

def check_a_result(line_part_two): # Apply on "3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green" for example
    data_cubes = line_part_two.split(";")
    game_validity = list(map(lambda x: compare_dict(get_results(x)), data_cubes))
    return all(game_validity)

#check_a_result("3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green")

def get_sum_id(text):
    lines = text.split("\n")
    games = list(map(lambda x: get_id(x.split(":")[0]) if (check_a_result(x.split(":")[1])) else 0 , lines))
    return sum(games)

print(get_sum_id(evaluation_text))