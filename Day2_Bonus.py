...
def keep_largest(revealed_cubes):
    max_dict = { "red":0, "blue":0, "green":0 }
    
    sets = revealed_cubes.replace(";",",").split(",")
    ...
    for index in range(len(cube_colors)):
        max_dict[cube_colors[index]] = max(max_dict[cube_colors[index]],number_of_cubes[index] )
    return max_dict
        
#print(keep_largest("3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green"))
    
def get_power(string):
    max_dict = keep_largest(string.split(":")[1])
    power = max_dict["red"]*max_dict["blue"]*max_dict["green"]
    return power

#print(get_power("""Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green"""))
      
def get_sum_powers(text):
    lines = text.split("\n")
    games_power = list(map(get_power , lines))
    return sum(games_power)

print(get_sum_powers(evaluation_text))