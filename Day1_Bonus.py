...
def get_line_number(string):
    first_digit = find_a_number(string, 0, len(string), 1)
    last_digit = find_a_number(string, len(string)-1, -1, -1)
...
def find_a_number(string, start, end, step): # Find the first number of iterative substring following the rules of start, end and step.
    for index in range(start, end, step):
        ...
        for number in numbers:
            ind_of_num = string.find(number, index, len(string))
            if index == ind_of_num:
                return number_dict[number]
            

evaluation_text="""threehqv2
...
print(get_calibration_value(evaluation_text))